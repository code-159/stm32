...
#include "stdio.h"
#include "delay.h"
u16 ov7670_zhen=0;						//帧中断的标志变量
u8 bug=0; 								//调试直接屏幕取颜色标志变量
u8  ov_frame=0;  						//帧率
u32 datanum=0;
u32 HSYNC=0;
u32 VSYNC=0;
...
void My_DCMI_Init(void)
{
  ...
  GPIO_Init(GPIOE, &GPIO_InitStructure);//初始化	
	...
	DCMI_InitStructure.DCMI_HSPolarity = DCMI_HSPolarity_Low;//HSYNC 低电平时数据无效 高电平时数据才有效
	DCMI_InitStructure.DCMI_PCKPolarity= DCMI_PCKPolarity_Falling;//PCLK 上升沿有效
	DCMI_InitStructure.DCMI_SynchroMode= DCMI_SynchroMode_Hardware;//硬件同步HSYNC,VSYNC
	DCMI_InitStructure.DCMI_VSPolarity=DCMI_VSPolarity_High;//VSYNC 低电平数据才有效  设置为高电平是数据无效
...
void DCMI_IRQHandler(void)
{
	...
	{
		DCMI_ClearITPendingBit(DCMI_IT_LINE);//清除中断	DCMI->ICR 位4置一
		ov_frame++;
	}
